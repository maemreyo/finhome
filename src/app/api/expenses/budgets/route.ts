...
export async function GET(request: NextRequest) {
  try {
    ...
    const period = searchParams.get('period') as 'weekly' | 'monthly' | 'yearly' | null
      ...
      budgets.map(async (budget) => {
        ...
        const spendingByCategory = transactions?.reduce((acc, t) => {
          if (t.expense_category_id) {
            acc[t.expense_category_id] = (acc[t.expense_category_id] || 0) + Number(t.amount)
          }
          return acc
        ...
        return {
          ...
          progress_percentage: (currentSpent / Number(budget.total_budget)) * 100,
        }
      })
    )
...
export async function POST(request: NextRequest) {
  try {
    ...
    const categoryBudgetSum = Object.values(categoryBudgets).reduce((sum, amount) => sum + amount, 0)
    ...
    if (categoryBudgets && Object.keys(categoryBudgets).length > 0) {
      const categoryAllocations = Object.entries(categoryBudgets).map(([categoryId, amount]) => ({
        budget_id: budget.id,
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json({ error: 'Invalid budget data', details: error.errors }, { status: 400 })
    }
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}