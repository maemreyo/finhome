...
const aiInsightRequestSchema = z.object({
  insight_type: z.enum(['spending_analysis', 'budget_optimization', 'saving_opportunities', 'goal_recommendations', 'comprehensive']).optional().default('comprehensive'),
  time_period: z.enum(['current_month', 'last_3_months', 'last_6_months', 'last_year']).optional().default('last_3_months'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const hasPremiumAccess = userProfile?.subscription_tier && 
    ...
    const financialData = await prepareFinancialDataForAI(supabase, user.id, validatedData.time_period)
...
async function prepareFinancialDataForAI(supabase: any, userId: string, timePeriod: string) {
  const periodMap = {
    'current_month': 1,
    'last_3_months': 3,
    'last_6_months': 6,
  ...
  const weeklySpending = expenses.reduce((acc, t) => {
  ...
  const budgetPerformance = budgets?.map(budget => ({
    ...
    performance_percentage: (Number(budget.total_spent) / Number(budget.total_budget)) * 100
  })) || []
  ...
  return {
    ...
    spending_patterns: {
      ...
      average_transaction: expenses.length > 0 ? totalExpenses / expenses.length : 0,
    ...
    budget_analysis: {
      ...
      over_budget_categories: budgetPerformance.filter(b => b.performance_percentage > 100),
      under_budget_categories: budgetPerformance.filter(b => b.performance_percentage < 80)
    },
    goal_analysis: {
      ...
      on_track_goals: goalProgress.filter(g => {
        const monthsToTarget = goal.target_date ? 
          Math.max(1, Math.ceil((new Date(goal.target_date).getTime() - Date.now()) / (30 * 24 * 60 * 60 * 1000))) : 12
...
async function generateAIInsights(financialData: any, requestParams: any): Promise<AIAnalysisResult> {
...
function buildFinancialAdvisorPrompt(financialData: any, requestParams: any): string {
  const { summary, spending_patterns, budget_analysis, goal_analysis } = financialData
...
${Object.entries(spending_patterns.by_category)
  ...
  .slice(0, 8)
  .map(([category, amount]) => `- ${category}: ${((amount as number) / 1000000).toFixed(1)}M VND`)
  .join('\n')}

CHI TIÊU LỚN NHẤT:
${spending_patterns.largest_expenses.map((exp: any) => 
  `- ${(exp.amount / 1000000).toFixed(1)}M VND: ${exp.description} (${exp.category})`
).join('\n')}

HIỆU SUẤT NGÂN SÁCH:
${budget_analysis.budget_performance.map((budget: any) => 
...
TIẾN ĐỘ MỤC TIÊU:
${goal_analysis.goal_progress.map((goal: any) => 
...
function validateAndEnhanceInsights(aiAnalysis: AIAnalysisResult, financialData: any): AIAnalysisResult {
  // Ensure all required fields exist and are valid
  const validated: AIAnalysisResult = {
    user_profile_summary: aiAnalysis.user_profile_summary || "Phân tích dữ liệu tài chính của bạn",
    user_profile_summary_vi: aiAnalysis.user_profile_summary_vi || aiAnalysis.user_profile_summary || "Phân tích dữ liệu tài chính của bạn",
    overall_financial_health_score: Math.min(100, Math.max(1, aiAnalysis.overall_financial_health_score || 50)),
    insights: (aiAnalysis.insights || []).map((insight, index) => ({
      ...
      insight_vi: insight.insight_vi || insight.insight || 'Phát hiện tài chính được AI tạo ra',
      actionable_steps: insight.actionable_steps || ['Review your spending'],
      actionable_steps_vi: insight.actionable_steps_vi || insight.actionable_steps || ['Xem lại chi tiêu của bạn'],
    ...
    recommendations: {
      immediate_actions: aiAnalysis.recommendations?.immediate_actions || ['Review your financial data'],
      immediate_actions_vi: aiAnalysis.recommendations?.immediate_actions_vi || aiAnalysis.recommendations?.immediate_actions || ['Xem lại dữ liệu tài chính'],
      long_term_strategies: aiAnalysis.recommendations?.long_term_strategies || ['Create a long-term financial plan'],
      long_term_strategies_vi: aiAnalysis.recommendations?.long_term_strategies_vi || aiAnalysis.recommendations?.long_term_strategies || ['Tạo kế hoạch tài chính dài hạn']
    },
...
function generateFallbackInsights(financialData: any): AIAnalysisResult {
  ...
  return {
    ...
    overall_financial_health_score: Math.max(30, Math.min(70, 50 + summary.savings_rate)),
    insights,
    recommendations: {
      immediate_actions: [
    ...
    personalized_tips: [
      {
        ...
        tip_vi: 'Thử quy tắc ngân sách 50/30/20: 50% nhu cầu, 30% mong muốn, 20% tiết kiệm',
        category: 'budgeting'
      }
    ]
  }
