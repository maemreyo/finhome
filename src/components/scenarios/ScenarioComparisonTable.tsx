...
const ScenarioComparisonTable: React.FC<ScenarioComparisonTableProps> = ({
  ...
  const t = useTranslations("ScenarioComparisonTable");
  ...
  const comparisonMetrics: ComparisonMetric[] = [
    ...
    },
    {
      ...
      icon: Calendar,
      format: (value) => `${value} ${t("months")}`,
      accessor: (scenario) =>
        scenario.calculatedMetrics?.payoffTimeMonths ||
        scenario.target_timeframe_months ||
        240,
      type: "months",
      importance: "medium",
    },
  ...
  const baselineScenario = useMemo(() => {
    return (
      selectedScenarios.find((s) => s.scenarioType === "baseline") ||
      selectedScenarios[0]
    );
  }, [selectedScenarios]);
  ...
  const getComparisonIndicator = (
    ...
    switch (type) {
      ...
      case "text":
        ...
        isImprovement =
          riskOrder[currentValue as keyof typeof riskOrder] <
          riskOrder[baselineValue as keyof typeof riskOrder];
        break;
      default:
        isImprovement = false;
    }
    ...
    return (
      <Card className={className}>
        <CardHeader>
          ...
          <CardDescription>
            {t('descriptionNoSelection')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-center py-8">
            <p className="text-gray-500">
              {t('noScenariosSelected')}
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <Card className={className}>
      <CardHeader>
        ...
        <CardDescription>
          {t('descriptionWithSelection', { count: selectedScenarios.length })}
        </CardDescription>
      </CardHeader>
      <CardContent>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-center gap-2">
                        <Badge
                          ...
                          className={cn(
                            "text-xs",
                            getScenarioTypeColor(scenario.scenarioType)
                          )}
                        >
              ...
              {comparisonMetrics.map((metric) => (
                <TableRow key={metric.key}>
                  ...
                  {selectedScenarios.map((scenario) => {
                    ...
                    const baselineValue = baselineScenario
                      ? metric.accessor(baselineScenario)
                      : value;
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-xs text-gray-600">
                      {t('totalCostLabel')} {formatCurrency(totalCost)}
                    </div>

                    {!isBest && costSavings > 0 && (
                      <div className="text-xs text-red-600">
                        {formatCurrency(costSavings)} {t('moreThanBestOption')}
                      </div>
                    )}

                    <div className="flex items-center gap-2 text-xs">
                      <span className="text-gray-500">{t('riskLabel')}</span>
                      <Badge
                        variant="outline"
                        className={cn(
                          "text-xs",
