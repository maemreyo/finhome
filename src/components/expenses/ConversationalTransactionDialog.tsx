...
interface Category {
  id: string; name_en: string; name_vi: string; icon: string; color: string; category_key: string;
}

interface Wallet {
  id: string; name: string; balance: number; currency: string; icon: string; color: string; wallet_type: string;
}
...
// --- Helper Components ---
const DynamicIcon = ({ name, ...props }: { name: string } & React.ComponentProps<typeof LucideIcons.HelpCircle>) => {
  const IconComponent = LucideIcons[name as keyof typeof LucideIcons] || LucideIcons.HelpCircle;
  return <IconComponent {...props} />;
};

function TransactionEditForm({
  transaction, onUpdate, onCancel, onSave, wallets, categories,
}: {
  transaction: TransactionFormData; onUpdate: (field: keyof TransactionFormData, value: any) => void;
  onCancel: () => void; onSave: () => void; wallets: Wallet[]; categories: Category[];
}) {
  return (
...
export function ConversationalTransactionDialog({
  isOpen, onClose, parsedData, wallets, expenseCategories, incomeCategories, onConfirm, originalText,
  ...
  useEffect(() => {
    ...
    if (parsedData?.transactions) {
      const initialTransactions = parsedData.transactions.map((t) => ({
        ...
        expense_category_id: t.transaction_type === "expense" ? t.suggested_category_id : undefined,
        income_category_id: t.transaction_type === "income" ? t.suggested_category_id : undefined,
  ...
  const { groupedTransactions, totalExpense, totalIncome, donutGradient } = useMemo(() => {
    const groups: Record<string, {
      category: Category | null | undefined; transactions: Array<{ original: ParsedTransaction; editing: TransactionFormData; index: number }>; totalAmount: number;
    }> = {};
    ...
    transactions.forEach((editing, index) => {
      ...
      if (!original) return;
      if (editing.transaction_type === 'expense') totalExpense += editing.amount;
      if (editing.transaction_type === 'income') totalIncome += editing.amount;

      const categories = editing.transaction_type === "expense" ? expenseCategories : incomeCategories;
      const categoryId = editing.transaction_type === "expense" ? editing.expense_category_id : editing.income_category_id;
      ...
      groups[groupKey].totalAmount += editing.transaction_type === "expense" ? -editing.amount : editing.amount;
    });
    ...
    const gradientParts = Object.values(groups)
      ...
      .map(group => {
        const percentage = totalExpense > 0 ? (Math.abs(group.totalAmount) / totalExpense) * 100 : 0;
    ...
    return { groupedTransactions: groups, totalExpense, totalIncome, donutGradient };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl w-full h-[85vh] flex flex-col p-0 bg-gray-50 overflow-hidden">
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-[380px_1fr] min-h-0">
          ...
          <div className="bg-gradient-to-b from-gray-100 to-gray-50 border-r flex flex-col">
            <div className="p-6 flex-shrink-0">
                <h2 className="text-lg font-semibold text-gray-800">Bảng phân tích</h2>
                <p className="text-sm text-gray-500 truncate">Từ: "{originalText}"</p>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center p-6 space-y-6 my-auto justify-center align-middle">
                <div className="relative w-48 h-48">
                    <div className="absolute inset-0 rounded-full" style={{ background: donutGradient }}></div>
                    <div className="absolute inset-2 bg-gray-100 rounded-full flex flex-col items-center justify-center shadow-inner">
                        ...
                        <p className={cn("text-3xl font-bold tracking-tighter", totalAmount >= 0 ? "text-green-600" : "text-red-600")}>
                ...
                <div className="grid grid-cols-2 gap-4 w-full">
                    <div className="bg-green-50 p-3 rounded-lg text-center">
                        ...
                        <p className="font-bold text-green-600">+{totalIncome.toLocaleString("vi-VN")}</p>
                    </div>
                    <div className="bg-red-50 p-3 rounded-lg text-center">
                        <p className="text-xs text-red-700">Tổng chi</p>
                        <p className="font-bold text-red-600">-{totalExpense.toLocaleString("vi-VN")}</p>
                    </div>
                </div>
            </div>
          </div>
              ...
              <div className="p-6 space-y-4">
                {Object.entries(groupedTransactions).map(([key, group]) => (
                    <div key={key} className="bg-white rounded-xl border border-gray-200/80 overflow-hidden">
                        <div className="p-3 border-b" style={{ backgroundColor: `${group.category?.color || '#cccccc'}1A` }}>
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{backgroundColor: `${group.category?.color || '#cccccc'}30`}}>
                                    <DynamicIcon name={group.category?.icon || "HelpCircle"} className="w-5 h-5" style={{color: group.category?.color || '#cccccc'}}/>
                                </div>
                                <div>
                                    <h3 className="font-semibold text-gray-800">{group.category?.name_vi || "Chưa phân loại"}</h3>
                                    <p className="text-sm text-gray-500">{group.transactions.length} giao dịch • {group.totalAmount.toLocaleString("vi-VN")} VND</p>
                                </div>
                            </div>
                        </div>
                        <div className="divide-y divide-gray-100">
                            {group.transactions.map(({ original, editing, index }) => {
                                if (editingIndex === index) {
                                    return <div className="p-2"><TransactionEditForm key={index} transaction={editing} onUpdate={(field, value) => updateTransaction(index, field, value)} onCancel={() => setEditingIndex(null)} onSave={() => setEditingIndex(null)} wallets={wallets} categories={editing.transaction_type === 'expense' ? expenseCategories : incomeCategories} /></div>;
                                }
                                const wallet = wallets.find(w => w.id === editing.wallet_id);
                                return (
                                <div key={index} className="p-3 flex items-center justify-between group hover:bg-gray-50/50">
                                    <div className="flex-1 min-w-0">
                                        <p className="font-medium truncate text-gray-800">{editing.description}</p>
                                    ...
                                    <div className="flex items-center gap-2 ml-4">
                                        <span className={cn("font-bold text-base", editing.transaction_type === 'expense' ? "text-red-600" : "text-green-600")}>
                                            {editing.transaction_type === 'expense' ? "-" : "+"} {editing.amount.toLocaleString("vi-VN")}
                                        </span>
                                        <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setEditingIndex(index)}><LucideIcons.Pencil className="h-4 w-4" /></Button>
                                            <Button variant="ghost" size="icon" className="h-8 w-8 text-red-500 hover:text-red-500 hover:bg-red-100" onClick={() => deleteTransaction(index)}><LucideIcons.Trash2 className="h-4 w-4" /></Button>
                                        </div>
                                    </div>
                                </div>
                                );
        ...
        <DialogFooter className="p-4 border-t bg-white/80 backdrop-blur-sm flex-shrink-0">
          ...
          <Button onClick={handleConfirm} disabled={isSubmitting || !transactions.length} className="bg-gray-800 hover:bg-gray-900 text-white font-semibold">
            {isSubmitting ? "Đang lưu..." : `Xác nhận & Lưu (${transactions.length})`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
