...
export class AdminQueries {
  ...
  static async createBank(
    bankData: Omit<Bank, "id" | "created_at" | "updated_at">
  ): Promise<Bank> {
    const { data, error } = await this.adminClient
      .from("banks")
      .insert({
  ...
  static async getAllInterestRates(): Promise<
    (BankInterestRate & { bank_name?: string; bank_code?: string })[]
  > {
    const { data, error } = await this.adminClient
  ...
  static async createInterestRate(
    rateData: Omit<BankInterestRate, "id" | "created_at" | "updated_at">
  ): Promise<BankInterestRate> {
    const { data, error } = await this.adminClient
      .from("bank_interest_rates")
      .insert({
        ...rateData,
  ...
  static async updateInterestRate(
    id: string,
    updates: Partial<BankInterestRate>
  ): Promise<BankInterestRate> {
    const { data, error } = await this.adminClient
      .from("bank_interest_rates")
      .update({
  ...
  static async createAchievement(
    achievementData: Omit<Achievement, "id" | "created_at" | "updated_at">
  ): Promise<Achievement> {
    const { data, error } = await this.adminClient
      .from("achievements")
      .insert({
  ...
  static async createNotification(
    notificationData: Omit<Notification, "id" | "created_at" | "read_at">
  ...
  // ===== BATCH OPERATIONS =====
  static async bulkUpdateBankStatus(
    bankIds: string[],
    isActive: boolean
  ): Promise<void> {
    const { error } = await this.adminClient
